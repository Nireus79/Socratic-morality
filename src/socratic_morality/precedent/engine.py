...
class MoralPrecedentEngine:
    ...
    async def find_similar_cases(
        self, action: str, limit: int = 5, threshold: float = 0.3
    ) -> List[Dict[str, Any]]:
        """
        ...
        for case in self.cases:
            ...
            if similarity >= threshold:
                similar.append({**case, "similarity_score": similarity})
    ...
    async def search_by_principle(self, principle: str) -> List[Dict[str, Any]]:
        ...
        return [case for case in self.cases if principle in case.get("principles_cited", [])]

    async def search_by_actor(self, actor: str) -> List[Dict[str, Any]]:
        """Search cases by actor."""
        return [case for case in self.cases if case.get("actor") == actor]

    async def get_decision_rate_for_principle(self, principle: str) -> Dict[str, Any]:
        """Get decision statistics for a principle."""
        principle_cases = await self.search_by_principle(principle)

        if not principle_cases:
            return {"principle": principle, "total_cases": 0, "allowed": 0, "denied": 0, "rate": 0}
        ...
        if self.embeddings and self.embeddings.is_available():
            ...
            if query_embedding:
                ...
                for case in self.cases:
                    ...
                    if case_embedding:
                        score = self.embeddings.cosine_similarity(query_embedding, case_embedding)
                        if score >= threshold:
                            similar.append(
                                {**case, "similarity_score": score, "similarity_method": "semantic"}
                            )

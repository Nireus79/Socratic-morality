...
class LLMEthicalAnalyzer:
    ...
    async def analyze_kantian(
        self, action: str, stakeholders: list, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        if self.llm_client:
    ...
    async def analyze_virtue_ethics(
        self, action: str, actor: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        if self.llm_client:
    ...
    async def _llm_analyze_kantian(
        self, action: str, stakeholders: list, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _llm_analyze_utilitarian(
        ...
        return result or {
            "allowed": True,
            "confidence": 0.8,
            "principle": "Harm Minimization",
            "concerns": None,
            "reasoning": "Unavailable",
        }

    async def _llm_analyze_virtue(
        self, action: str, actor: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        prompt = f"Analyze: {action}"
        response = await self.llm_client.messages.create(
    ...
    def _fallback_analyze_kantian(action: str, stakeholders: list) -> Dict[str, Any]:
        ...
        return {
            "allowed": not has_v,
            "confidence": 0.75 if has_v else 0.6,
            "principle": "Categorical Imperative",
            "concerns": "Violates" if has_v else None,
            "reasoning": "Complete",
        }
    ...
    def _fallback_analyze_utilitarian(action: str) -> Dict[str, Any]:
        ...
        return {
            "allowed": not has_h,
            "confidence": 0.7 if has_h else 0.8,
            "principle": "Harm Minimization",
            "concerns": "Harm" if has_h else None,
            "reasoning": "Complete",
        }
    ...
    def _fallback_analyze_virtue(action: str) -> Dict[str, Any]:
        ...
        return {
            "allowed": not has_v,
            "confidence": 0.8 if has_v else 0.6,
            "principle": "Moral Character",
            "concerns": "Vice" if has_v else None,
            "reasoning": "Complete",
        }
    ...
    def _fallback_analyze_rights(action: str) -> Dict[str, Any]:
        ...
        return {
            "allowed": not has_v,
            "confidence": 0.85 if has_v else 0.8,
            "principle": "Human Agency",
            "concerns": "Autonomy" if has_v else None,
            "reasoning": "Complete",
        }

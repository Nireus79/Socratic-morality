...
class ExplanationGenerator:
    ...
    def generate_explanation(
        ...
        precedent_cases: Optional[List[Dict[str, Any]]] = None,
    ) -> str:
        parts = []
        ...
        frameworks_summary = self._generate_frameworks_summary(
            decision_result.get("frameworks", {})
        )
    ...
    def _generate_headline(allowed: bool, confidence: float) -> str:
        if allowed:
            if confidence >= 0.9:
                return "DECISION: Approved with high confidence."
            elif confidence >= 0.7:
                return "DECISION: Approved with some concerns."
            else:
                return "DECISION: Approved but with low confidence."
        else:
            if confidence >= 0.9:
                return "DECISION: Denied with high confidence."
            elif confidence >= 0.7:
                return "DECISION: Denied; concerns outweigh benefits."
            else:
                return "DECISION: Denied but with low confidence."
    ...
    def _generate_stakeholder_analysis(stakeholders: List[str]) -> str:
        ...
        for stakeholder in stakeholders:
            if stakeholder.startswith("user:"):
                text += f"* User: {stakeholder[5:]} (direct impact)\n"
            elif stakeholder.startswith("org:"):
                text += f"* Organization: {stakeholder[4:]} (institutional)\n"
            else:
                text += f"* {stakeholder}\n"
        return text.rstrip()
    ...
    def _generate_confidence_statement(confidence: float) -> str:
        ...
        if confidence >= 0.9:
            return f"Confidence: {confidence_pct}% (Very High)"
        elif confidence >= 0.7:
            return f"Confidence: {confidence_pct}% (High)"
        elif confidence >= 0.5:
            return f"Confidence: {confidence_pct}% (Moderate)"
        else:
            return f"Confidence: {confidence_pct}% (Low)"
